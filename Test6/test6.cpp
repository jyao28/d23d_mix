...
void d3d11_engine::create_shared_texture2d()
{
   texture2 = create_texture2d("rose.jpg", D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, D3D11_RESOURCE_MISC_SHARED);

   {
      IDXGISurface* surface = nullptr;
      AssertHResult(texture2->QueryInterface(__uuidof(IDXGISurface), (void**)(&surface)), "Failed to get surface");
...
void d3d11_engine::init(HWND hwnd)
{
   ...
   create_texture2d();
   create_shared_texture2d();
}

void d3d11_engine::update_image()
{
   if (!shared_texture)
   {
      ...
      {
         HANDLE resource_handle = shared_handle();
